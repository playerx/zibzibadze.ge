...
OrgChart.prototype.load = function (t) {
    ...
    OrgChart.prototype._draw = function (t, m, O, y) {
        ...
        this.server.read(t, this.width(), this.height(), e, m, O, function (t) {
            v.nodes = t.nodes,
                v.editUI.fields = t.allFields;
            var e = v.ui.defs()
                , r = v.getScale(t.viewBox);
            e += v.ui.pointer(v.config, m, r);
            var i = v.getViewBox()
                , a = t.viewBox;
            for (var n in v._initialized || (v.element.innerHTML = '<div id="bg-tests"></div>'),
                t.visibleNodes) {
                var o = t.visibleNodes[n];
                e += v.ui.node(o, v._get(n), t.animations, v.config, void 0, void 0, void 0, m, r, v),
                    e += v.ui.secondLinks(t.visibleNodes, o, v.config, t.boundary, r),
                    e += v.ui.link(o, v, v.config, r),
                    e += v.ui.expandCollapse(o, v.config, m, r)
            }
            if (e += v.ui.lonely(v.config),
                m !== OrgChart.action.centerNode && m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update || (a = i),
                m === OrgChart.action.init && null != i && (a = i),
                m !== OrgChart.action.exporting) {
                v.response = t;
                C = v.ui.svg(v.width(), v.height(), a, v.config, e);
                if (v._initialized) {
                    var l = v.getSvg()
                        , s = l.parentNode;
                    s.removeChild(l),
                        s.insertAdjacentHTML("afterbegin", C),
                        v._attachEventHandlers(),
                        v.xScrollUI.addListener(v.getSvg()),
                        v.yScrollUI.addListener(v.getSvg()),
                        v.xScrollUI.setPosition(),
                        v.yScrollUI.setPosition()
                } else {
                    v.element.innerHTML = v.ui.css() + v.ui.exportMenuButton(v.config) + C,
                        v._attachInitEventHandlers(),
                        v._attachEventHandlers(),
                        v.config.showXScroll === OrgChart.scroll.visible && (v.xScrollUI.create(v.width(), v.config.padding),
                            v.xScrollUI.setPosition(),
                            v.xScrollUI.addListener(v.getSvg())),
                        v.config.showYScroll === OrgChart.scroll.visible && (v.yScrollUI.create(v.height(), v.config.padding),
                            v.yScrollUI.setPosition(),
                            v.yScrollUI.addListener(v.getSvg())),
                        v.config.enableSearch && v.searchUI.addSearchControl(),
                        v.toolbarUI.init(v, v.config.toolbar);
                    var h = document.createElement("div");
                    h.setAttribute("id", "bg-tests"),
                        v.element.appendChild(h)
                }
                var d = !1;
                v.notifierUI.show(t.notif);
                var c = v.response.animations;
                if (0 < c[0].length) {
                    v._hideBeforeAnimation(c[0].length);
                    for (var g = 0; g < c[0].length; g++)
                        c[0][g] = v.getNodeElement(c[0][g]);
                    OrgChart.anim(c[0], c[1], c[2], v.config.anim.duration, v.config.anim.func, function () {
                        d || (y && y(),
                            OrgChart.events.publish("redraw", [v]),
                            v._showAfterAnimation(),
                            d = !0)
                    })
                }
                m === OrgChart.action.centerNode ? OrgChart.anim(v.getSvg(), {
                    viewbox: i
                }, {
                        viewbox: v.response.viewBox
                    }, v.config.anim.duration, v.config.anim.func, function () {
                        v.ripple(O.options.rippleId)
                    }, function () {
                        v.xScrollUI.setPosition(),
                            v.yScrollUI.setPosition(),
                            d || (y && y(),
                                ...
                                v._showAfterAnimation(),
                                d = !0)
                    }) : !i || !v.response || i[0] == v.response.viewBox[0] && i[1] == v.response.viewBox[1] && i[2] == v.response.viewBox[2] && i[3] == v.response.viewBox[3] || m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update && m !== OrgChart.action.init ? 0 == c[0].length && (d || (y && y(),
                        OrgChart.events.publish("redraw", [v]),
                        d = !0)) : OrgChart.anim(v.getSvg(), {
                            viewbox: i
                        }, {
                                viewbox: v.response.viewBox
                ...
                var p = t.boundary
                    , f = p.maxX - p.minX
                    , u = p.maxY - p.minY
                    , C = v.ui.svg(f, u, [p.minX, p.minY, f, u], v.config, e, r);
                y(C)
            }
        }, function (t) {
            OrgChart.events.publish("ready", [v, t])
        })
    }
    ,
    ...
    OrgChart.prototype.getScale = function (t) {
        return t || (t = this.getViewBox()),
            OrgChart.getScale(t, this.width(), this.height(), this.config.scaleInitial)
    }
    ,
    OrgChart.prototype.ripple = function (t, e, r) {
        ...
        if (null != i) {
            ...
            if (null != a) {
                ...
                C.setAttribute("x", y.ripple.rect ? y.ripple.rect.x : 0),
                    C.setAttribute("y", y.ripple.rect ? y.ripple.rect.y : 0),
                    C.setAttribute("width", y.ripple.rect ? y.ripple.rect.width : i.w),
                    C.setAttribute("height", y.ripple.rect ? y.ripple.rect.height : i.h),
                    C.setAttribute("rx", y.ripple.radius),
                    C.setAttribute("ry", y.ripple.radius),
                    m.setAttribute("clip-path", "url(#" + O + ")"),
                    m.setAttribute("cx", o),
                    m.setAttribute("cy", l),
                    m.setAttribute("r", 0),
                    m.setAttribute("fill", y.ripple.color),
                    u.appendChild(C),
                    f.appendChild(u),
                    f.appendChild(m),
                    a.appendChild(f),
        ...
        if (this._canUpdateLink(t, e)) {
            ...
            if (r.isGroup && i.isGroup)
                for (var a = i.bgnodes[0].id, n = 0; n < r.bgnodes.length; n++) {
                    (o = this.get(r.bgnodes[n].id)).pid = a,
                        this.update(o, !0)
                }
                ...
                for (n = 0; n < r.bgnodes.length; n++) {
                    (o = this.get(r.bgnodes[n].id)).pid = e,
                        this.update(o, !0)
                }
            else if (!r.isGroup && !i.isGroup) {
                var o;
    ...
    OrgChart.prototype.update = function (t, e) {
        var r = this.get(t.id);
        if (!0 === e && this.config.onUpdate && !1 === OrgChart.events.publish("update", [this, r, t]))
            return !1;
    ...
    OrgChart.prototype.group = function (t, e) {
        ...
        if (r.isGroup || i.isGroup || r.isChildOfGroup || i.isChildOfGroup)
            if (r.isGroup || i.isGroup || !r.isChildOfGroup || i.isChildOfGroup)
                if (r.isGroup || i.isGroup || r.isChildOfGroup || !i.isChildOfGroup)
                    !r.isGroup && !i.isGroup && r.isChildOfGroup && i.isChildOfGroup && (this._removeGroupIfHasOneNode(r._groupParentNodeId, r.id),
                ...
                if (r.id != l)
                    ...
                    for (var s = this.getBGNode(r._groupParentNodeId), h = (l = "",
                        0); h < s.bgnodes.length; h++) {
                        var d = s.bgnodes[h];
                        if (d.id != r.id) {
                            l = d.id;
                            break
                ...
                for (var t = 0; t < c.length; t++)
                    for (var e in p[t]) {
                        ...
                        switch (e.toLowerCase()) {
                            case "d":
                                var a = u((10 * y - 10) / f) * (p[t][e][0] - g[t][e][0]) + g[t][e][0]
                                    , n = u((10 * y - 10) / f) * (p[t][e][1] - g[t][e][1]) + g[t][e][1];
                                c[t].setAttribute("d", c[t].getAttribute("d") + " L" + a + " " + n);
                                break;
                            case "r":
                                var o = u((10 * y - 10) / f) * (p[t][e] - g[t][e]) + g[t][e];
                            ...
                            case "margin":
                                ...
                                break;
                            default:
                                o = u((10 * y - 10) / f) * (p[t][e] - g[t][e]) + g[t][e],
                                    c[t] && c[t].style && (c[t].style[e] = o + r)
                        }
                    }
                m && m(),
                    1 + v < (y += 1) && (clearInterval(O),
                        C && C(c))
            }, 10)
    }
    ,
    ...
    OrgChart.prototype._attachEventHandlers = function (t) {
        t = this.getSvg();
        /Mobi/i.test(navigator.userAgent) || /Android/i.test(navigator.userAgent) ? (this._addEvent(t, "touchstart", this._globalMouseDownHandler),
            this._addEvent(t, "touchend", this._globalClickHandler)) : (this._addEvent(t, "mousedown", this._globalMouseDownHandler),
                ...
                this.config.mouseScrool == OrgChart.action.zoom && (this._addEvent(t, "DOMMouseScroll", this._mouseScrollHandler),
    ...
    OrgChart.input = function (t, e, r, i, a) {
        ...
        if (h.innerHTML = "Upload",
            ...
            i || l.addEventListener("focus", function () {
    ...
    OrgChart._intersects = function (t, e, r) {
        ...
        return 0 != (l = (n - i) * (p - c) - (g - d) * (o - a)) && (s = ((a - o) * (g - i) + (n - i) * (p - a)) / l,
    ...
    OrgChart.editUI.prototype._createDetailsView = function (t, e) {
        ...
        for (var h = 0; h < l.length; h++) {
            ...
            if (t.isGroup && (d = this.obj.config.tags[t.id]["group" + l[h]]),
                ...
                n.appendChild(OrgChart.input(l[h], d, null, !0))
        }
            ...
            OrgChart.anim(this.wrapperElement, {
                ...
                opacity: 0
                    ...
                    r.obj._showAfterAnimation(),
                        n.style.height = r.obj.element.offsetHeight - i.offsetHeight + "px"
                })
    }
    ,
    OrgChart.editUI.prototype._createEditView = function (t, e) {
        ...
        l.innerHTML = '<div label="isAssistant" style="margin-top: 10px;display:inline-block;">Assistant</div><label class="bg-switch"><input type="checkbox" ' + s + '><span class="bg-slider round"></span></label>',
            this.wrapperElement.appendChild(e),
            e.appendChild(r),
            e.appendChild(o),
            r.appendChild(a),
        ...
        for (var c = 0; c < h.length; c++) {
            ...
            if (t.isGroup && (g = this.obj.config.tags[t.id]["group" + h[c]]),
                ...
                if (OrgChart._fieldIsImg(this.obj.config, h[c])) {
                    ...
                    o.appendChild(OrgChart.input(h[c], g, null, !1))
        }
        t.isGroup || t.isChildOfGroup || 0 != t.childrenIds.length || o.appendChild(l),
            ...
            OrgChart.anim(this.wrapperElement, {
                ...
                opacity: 0
                    ...
                    i.obj._showAfterAnimation(),
                        o.style.height = i.obj.element.offsetHeight - r.offsetHeight + "px",
    ...
    OrgChart.menuUI.prototype.show = function (t, e, n, o, l) {
        ...
        for (var i in l || (l = this.menu),
            ...
            void 0 === a && (a = OrgChart.icon[i](24, 24, "#7A7A7A")),
                ...
                r += '<div data-item="' + i + '" style="border-bottom: 1px solid #D7D7D7; padding: 7px 10px;color: #7A7A7A;">' + a + "<span>&nbsp;&nbsp;" + h + "</span></div>"
        }
        if ("" != r) {
                    ...
                    i.addEventListener("click", function (t) {
                        ...
                        if (void 0 === l[r].onClick)
                            if ("add" === r) {
                                ...
                                "remove" === r ? s.obj.removeNode(n, !0) : "svg" === r ? s.obj.exportSVG({
    ...
    OrgChart.toolbarUI.prototype.init = function (t, e) {
        if (e) {
            this.obj = t,
                ...
                e.fit && (this.div.innerHTML += '<svg data-tlbr="fit" style="margin-bottom:7px;box-shadow: 0px 1px 4px rgba(0,0,0,0.3); border: 1px solid #cacaca;background-color: #f9f9f9;display: block;cursor: pointer;" width="32px" height="32px"><path stroke-width="3" fill="none" stroke="#757575" d="M4,11 L4,4 L11,4"></path><path stroke-width="3" fill="none" stroke="#757575" d="M28,11 L28,4 L21,4"></path><path stroke-width="3" fill="none" stroke="#757575" d="M28,21 L28,28 L21,28"></path><path stroke-width="3" fill="none" stroke="#757575" d="M4,21 L4,28 L11,28"></path><circle cx="16" cy="16" r="5" fill="#757575"></circle></svg>'),
                e.zoom && (this.div.innerHTML += '<svg style="box-shadow: 0px 1px 4px rgba(0,0,0,0.3); margin-bottom:7px;border: 1px solid #cacaca; background-color: #f9f9f9;display: block; cursor: pointer;" width="32px" height="64px" ><g data-tlbr="plus"><rect fill="#f9f9f9" x="0" y="0" width="32" height="32" ></rect><line x1="8" y1="16" x2="24" y2="16" stroke-width="3" stroke="#757575"></line><line x1="16" y1="8" x2="16" y2="24" stroke-width="3" stroke="#757575"></line></g><line x1="4" y1="32" x2="28" y2="32" stroke-width="1" stroke="#cacaca"></line><g data-tlbr="minus"><rect fill="#f9f9f9" x="0" y="32" width="32" height="32" ></rect><line x1="8" y1="48" x2="24" y2="48" stroke-width="3" stroke="#757575"></line></g></svg>'),
                e.layout && (this.div.innerHTML += '<svg data-tlbr="layout" style="box-shadow: 0px 1px 4px rgba(0,0,0,0.3); border: 1px solid #cacaca;background-color: #f9f9f9;display: block;cursor: pointer;" width="32px" height="32px"><path stroke-width="3" fill="none" stroke="#757575" d="M8,24 L16,14 L24,24"></path><path stroke-width="3" fill="none" stroke="#757575" d="M8,16 L16,8 L24,16"></path></svg>',
        ...
        this._draw(!1, OrgChart.action.exporting, t, function (t) {
            ...
            if (r.innerHTML = t,
                ...
                if (a)
                    ...
                    if (!1 === e)
                        return !1;
                    if (OrgChart._downloadFile(i.mime, r.innerHTML, i.filename),
                        !1 === (e = OrgChart.events.publish("exportend", [n, i, r.innerHTML])))
                        return !1
                }
            else
                OrgChart._imgs2base64(r, "image", "xlink:href", function () {
                    var t = OrgChart._pages(i, r.querySelector("svg"));
                    if (!1 === OrgChart.events.publish("exportstart", [n, i, r.querySelector("svg"), t]))
                        return !1;
                    ...
                    a ? a(n, e, r.querySelector("svg")) : (e = JSON.stringify(e),
                        OrgChart._ajax(n.config.exportUrl + "/v2", "POST", e, "arraybuffer", function (t) {
                            if (!1 === OrgChart.events.publish("exportend", [n, i, t]))
                                return !1;
    ...
    ,
    OrgChart._pages = function (t, e) {
        return "A4" == t.format && 100 == t.scale ? OrgChart._pagesA4100(t, e) : "A4" == t.format && "fit" == t.scale ? OrgChart._pagesA4fit(t, e) : "fit" == t.format ? OrgChart._pagesFit(t, e) : void 0
    }
    ,
    OrgChart._pagesFit = function (t, e) {
    ...
    ,
    OrgChart._pagesA4fit = function (t, e) {
        var r = e.getAttribute("width")
            , i = e.getAttribute("height")
            , a = OrgChart.pdfPrevUI._getViewBox(e)
            , n = {
                w: t.landscape ? OrgChart.A4h - (t.margin[1] + t.margin[3]) : OrgChart.A4w - (t.margin[1] + t.margin[3]),
                h: t.landscape ? OrgChart.A4w - (t.margin[0] + t.margin[2]) : OrgChart.A4h - (t.margin[0] + t.margin[2])
            }
            , o = {
                w: t.landscape ? OrgChart.A4h : OrgChart.A4w,
                h: t.landscape ? OrgChart.A4w : OrgChart.A4h
            }
            , l = r / n.w
            , s = a[0]
            , h = a[1] / l
            , d = a[2]
            , c = a[3] / l;
        e.setAttribute("width", n.w),
            e.setAttribute("height", n.h),
            e.setAttribute("preserveAspectRatio", "xMidYMin meet");
        for (var g = n.w, p = n.h, f = s, u = h, C = []; u < c;) {
            var m = [f, u * l, d, c];
            m = m.join(),
                C.push({
                    vb: m,
    ...
    ,
    OrgChart._pagesA4100 = function (t, e) {
        var r = e.getAttribute("width")
            , i = e.getAttribute("height")
            , a = OrgChart.pdfPrevUI._getViewBox(e)
            , n = a[0]
        ...
        e.setAttribute("preserveAspectRatio", "xMinYMin slice"),
            e.setAttribute("width", h.w),
            e.style.display = "";
        for (var c = h.w, g = h.h, p = n, f = o, u = []; p < l;) {
            for (; f < s;) {
                var C = [p, f, l, s];
                C = C.join(),
                    u.push({
            ...
            publish: function (t, e) {
                if (n.call(a, t) && a[t])
                    for (var r = 0; r < a[t].length; r++) {
                        var i = a[t][r];
                        if (!i.event_id || i.event_id == e[0]._event_id) {
                            if (1 == e.length)
                                return i.listener(e[0]);
                            if (2 == e.length)
                                return i.listener(e[0], e[1]);
                            if (3 == e.length)
                                return i.listener(e[0], e[1], e[2]);
                            if (4 == e.length)
                                return i.listener(e[0], e[1], e[2], e[3]);
                            if (5 == e.length)
                                return i.listener(e[0], e[1], e[2], e[3], e[4])
                        }
                    }
            }
        }
    ...
    OrgChart.prototype._nodeMouseDownHandler = function (t, e, i) {
        if (this.config.enableDragDrop) {
            ...
            var h = this
                ...
                , u = h.getScale()
                , C = t.cloneNode(!0);
            if (d.insertBefore(C, d.firstChild),
                C.style.opacity = .7,
                n.isChildOfGroup) {
                ...
                p += m[4],
                    ...
                    g[4] = p,
                    g[5] = f,
                    C.setAttribute("transform", "matrix(" + g.toString() + ")")
            }
            var O = function (t) {
                var e, r = OrgChart._getClientXY(t), i = document.elementFromPoint(r.x, r.y);
                if (null != l && ((e = h.getBGNode(l)).isChildOfGroup ? (h.getNodeElement(e._groupParentNodeId).style.opacity = 1,
                    s.style.opacity = 1) : e.isGroup || (s.style.opacity = 1)),
                    s = l = null,
                    h.config.enableDragDrop)
                    for (; null != i && i != d;) {
                        if (i.hasAttribute("node-id")) {
                ...
                null != l && ((e = h.getBGNode(l)).isChildOfGroup ? (h.getNodeElement(e._groupParentNodeId).style.opacity = .8,
                    s.style.opacity = .5) : e.isGroup ? s = l = null : s.style.opacity = .5);
                var n = (r.x - c.x) / u
                    , o = (r.y - c.y) / u;
                g[4] = p + n,
                    g[5] = f + o,
                ...
                , y = function (t) {
                    if (d.removeEventListener(i.move, O),
                        d.removeEventListener(i.up, y),
                        i.leave && d.removeEventListener(i.leave, y),
                        n.id == l || n.isChildOfGroup && n._groupParentNodeId == l || null == l)
                        return d.removeChild(C),
                            void (h._gragStartedId = null);
                    var e = h.getBGNode(l);
                    if (null == h.config.dragDropMenu || n.isGroup && !e.isGroup)
                        h._canUpdateLink(n.id, l) ? h.linkNode(n.id, l) : d.removeChild(C);
            ...
            d.addEventListener(i.move, O),
                d.addEventListener(i.up, y),
                i.leave && d.addEventListener(i.leave, y)
        }
    }
    ,
    ...
    OrgChart.prototype._nodeClickHandler = function (t, e) {
        if (!1 === OrgChart.events.publish("click", [this, this.get(t)]))
            return !1;
    ...
    OrgChart.remote._walk = function (r, t, i) {
        for (var e = [], a = [], n = [t.orientation, t.levelSeparation, t.mixedHierarchyNodesSeparation, t.subtreeSeparation, t.siblingSeparation, t.template], o = 0; o < r.length; o++)
            a.push(r[o].id),
                OrgChart.remote._toReqDTO(r[o], e, t.template);
        var l = {
            n: e,
            c: n,
            r: a,
        ...
        OrgChart.LIMIT_NODES || (l.l = !0),
            null != OrgChart.remote._fromReqDTO ? OrgChart.remote._fromReqDTO(l.n, l.r, l.c, function (t) {
                ...
                i()
            }) : (l = JSON.stringify(l).replace(/\"([^(\")"]+)\":/g, "$1:"),
    ...
    OrgChart.remote._findRegion = function (r) {
        ...
        if (t)
            ...
            for (var i = ["au-e", "au-se", "brs", "ca", "ca-e", "easia", "eus-2", "eus", "fr", "ind", "jp-e", "jp-w", "kr", "n-eu", "se-asia", "s-ind", "uk-s", "uk-w", "us", "us-n-c", "us-s-c", "w-c-us", "w-eu", "w-ind", "w-us-2", "wus"], a = [], n = 0; n < i.length; n++)
                ...
                !function () {
                    ...
                    t.onreadystatechange = function () {
                        if (4 == this.readyState && 200 == this.status) {
                            localStorage.setItem("funcUrl", e),
                                r(e);
        ...
        for (s = 0; s < o.length && 10 != s; s++) {
            ...
            if (!g) {
                ...
                for (p = 0; p < O.length; p++) {
                    d = O[p],
                        c = o[s].data[d];
                    d === o[s].propertyName && (i = t,
                        void 0,
                        a = (e = c).toString().toLowerCase().indexOf(i),
                        c = e.toString().splice(a + i.length, 0, "</mark>").splice(a, 0, "<mark>")),
                        d != OrgChart.TAGS && (d == OrgChart.ID ? u = u + c + ", " : OrgChart._fieldIsImg(this.config, d) ? m = c : n(d) && !OrgChart._fieldIsImg(this.config, d) ? f = f + c + ", " : OrgChart._fieldIsImg(this.config, d) || (C = C + c + ", "))
                }
    ...
    OrgChart.server._createNodes = function (t, e, r, i) {
        for (var a = t.nodes, n = {}, o = [], l = [], s = [], h = {}, d = !1, c = 0; c < a.length; c++) {
            ...
            if ((v = new OrgChart.node(g.id, g.pid, C, p, u)).isAssistant = -1 != p.indexOf("assistant"),
                v.isAssistant && -1 == s.indexOf(v.pid) && s.push(v.pid),
                null != f) {
                if (!n[f.name]) {
                    o.push(f.name);
                ...
                v.isChildOfGroup = !0,
                    ...
                    n[f.name].bgnodes.push(v),
                    h[v.id] = f.name,
                    d = !0
            } else
                null != t.orderBy && (v.order = g[t.orderBy]),
                    n[g.id] = v,
                    o.push(g.id)
        }
        if (null != t.orderBy && o.sort(function (t, e) {
            ...
            return "number" == typeof r || "number" == typeof i ? (null == r && (r = -1),
                null == i && (i = -1),
                r - i) : "string" == typeof r || "string" == typeof i ? (null == r && (r = ""),
                    null == i && (i = ""),
                    r.localeCompare(i)) : void 0
        }),
            d)
            for (c = 0; c < o.length; c++) {
                h[(v = n[o[c]]).pid] && (v.pid = h[v.pid])
            }
        var O = [];
        for (c = 0; c < o.length; c++) {
            var y = o[c]
                , v = n[y]
                , x = i ? i[y] : null;
            if (null != v.pid && null != v.pid && "" !== v.pid)
                (_ = n[v.pid]) ? ((v.parent = _).children.push(v),
                    _.childrenIds.push(v.id)) : (v.pid = null,
                        v.parent = null,
                        l.push(v));
            else
                l.push(v);
            e == OrgChart.action.exporting && null != r.id && (l = [n[r.id]]),
                x ? (v.state = x.state,
                    v.state == OrgChart.COLLAPSE && O.push(v.id),
                    OrgChart.server._setNodeGroupState(v, x.groupState, t.orientation)) : OrgChart.server._setNodeGroupState(v, OrgChart.EXPAND, t.orientation),
                OrgChart.server._initStates(v, n, e, t, r, h, O),
                OrgChart.server._initGroupStates(v, n, e, t, r)
        }
        if (null != t.roots) {
            l = [];
                ...
                if (b) {
                    if (b.parent)
                        for (var w = (_ = b.parent).children.length - 1; 0 <= w; w--) {
                            (j = _.children[w]) == b && (j.parent = null,
        ...
        for (c = 0; c < s.length; c++) {
            ...
            (x = i[(n[k.id] = k).id]) && (n[k.id].state = x.state,
                n[k.id].state == OrgChart.COLLAPSE && O.push(k.id)),
                OrgChart.server._initStates(k, n, e, t, r, h, O);
            var S = [];
            for (w = _.children.length - 1; 0 <= w; w--) {
                (j = _.children[w]).isAssistant ? (j.parent = null,
                    _.children.splice(w, 1),
                    _.childrenIds.splice(w, 1),
                    S.splice(0, 0, j.id)) : (j.pid = k.id,
                        (j.parent = k).children.push(j),
                        k.childrenIds.push(j.id),
                        _.children.splice(w, 1))
            }
            if (S.length % 2) {
                var N = n[S[S.length - 1]]
                    , A = OrgChart.t(N.templateName)
                    , I = "transperant_" + N.templateName;
                OrgChart.templates[I] = {
                    ...
                    (J = new OrgChart.node(_.id + "_transperant", N.pid, null, [], I)).isAssistant = !0,
                    (x = i[(n[J.id] = J).id]) && (n[J.id].state = x.state,
                        n[J.id].state == OrgChart.COLLAPSE && O.push(J.id)),
                    OrgChart.server._initStates(J, n, e, t, r, h, O),
                    S.splice(S.length - 1, 0, J.id)
            }
            var E = 1;
            for (w = S.length - 1; 0 <= w; w--)
                if (w % 2 && w != S.length - 1) {
                    var M = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split");
                    (x = i[(n[M.id] = M).id]) && (n[M.id].state = x.state,
                        n[M.id].state == OrgChart.COLLAPSE && O.push(M.id)),
                        OrgChart.server._initStates(M, n, e, t, r, h, O),
                        S.splice(w, 0, M.id),
                        E++
                } else
                    w % 2 && S.splice(w, 0, k.id);
            for (w = 0; w < S.length; w += 3) {
                ...
                B.parent = L,
                    ...
                    L.children.push(T),
                    L.children.push(P),
                    L.childrenIds.push(B.id),
                    L.childrenIds.push(T.id),
                    L.childrenIds.push(P.id);
                for (var H = L; "split" == H.templateName && (H = H.parent);)
                    H.childrenIds.push(B.id),
                        H.childrenIds.push(T.id),
                        H.childrenIds.push(P.id)
            }
        }
        if (e == OrgChart.action.centerNode) {
            var z = n[r.id];
            z || (z = n[h[r.id]],
                OrgChart.server._setNodeGroupState(z, OrgChart.EXPAND, t.orientation)),
                z.state = OrgChart.EXPAND,
                -1 != (G = O.indexOf(z.id)) && O.splice(G, 1);
            _ = OrgChart.server._getRealParent(z);
            for (var U = z.id; null != _;) {
                if (-1 != (G = O.indexOf(_.id)) && O.splice(G, 1),
                    _.state = OrgChart.EXPAND,
                    r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
                    for (c = 0; c < _.childrenIds.length; c++) {
                        var j = n[_.childrenIds[c]];
                        if (!OrgChart.server._isSplit(j) && !j.isAssistant && j.id != U)
                            -1 == (G = O.indexOf(_.childrenIds[c])) && O.push(_.childrenIds[c]),
                                j.state = OrgChart.COLLAPSE
                    }
                if (_.state == OrgChart.EXPAND)
                    for (var F = 0; F < _.childrenIds.length; F++) {
                        var D = n[_.childrenIds[F]];
                        if (OrgChart.server._isSplit(D) || D.isAssistant)
                            -1 != (G = O.indexOf(D.id)) && O.splice(G, 1),
                                D.state = OrgChart.EXPAND
                    }
                U = _.id,
                    _ = OrgChart.server._getRealParent(_)
            }
            if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
                for (c = 0; c < z.childrenIds.length; c++) {
                    if (-1 != (G = O.indexOf(z.childrenIds[c])) && O.splice(G, 1),
                        (j = n[z.childrenIds[c]]).state = OrgChart.EXPAND,
                        !j.isAssistant && !OrgChart.server._isSplit(j))
                        for (w = 0; w < j.childrenIds.length; w++) {
                            var G, R = n[j.childrenIds[w]];
                            -1 == (G = O.indexOf(j.childrenIds[w])) && O.push(j.childrenIds[w]),
                                R.state = OrgChart.COLLAPSE
                        }
                }
        }
        e == OrgChart.action.exporting && 1 == r.expandChildren && v && (O = []);
        for (c = 0; c < O.length; c++) {
            if (_ = (v = n[O[c]]).parent)
                for (w = _.children.length - 1; 0 <= w; w--) {
                    if (_.children[w] == v)
                        if (-1 == _.collapsedChildrenIds.indexOf(v.id) && _.collapsedChildrenIds.push(v.id),
                            _.children.splice(w, 1),
                            "split" == v.templateName || "split" == _.templateName) {
                            if (L = v.parent) {
                                for (; "split" == L.templateName;)
                                    L = L.parent;
                                for (var X = 0; X < L.childrenIds.length; X++) {
                                    var q = L.childrenIds[X];
                                    -1 == L.collapsedChildrenIds.indexOf(q) && L.collapsedChildrenIds.push(q)
                                }
                            }
                        } else
                            v.parent = null
        ...
        if (t.layout == OrgChart.mixed)
            for (c = 0; c < o.length; c++) {
                if (0 == (v = n[o[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
                    if (_ = v.parent)
                        for (w = _.children.length - 1; 0 <= w; w--) {
                            if ((j = _.children[w]) != v && 0 == j.childrenIds.length && !j.isAssistant && "split" != j.templateName) {
                                j.parent = v,
                                    j.pid = v.id,
        ...
        if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
            ...
            for (c = 0; c < o.length; c++) {
                if (0 == (v = n[o[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
                    (_ = v.parent) && -1 == V.indexOf(_.id) && V.push(_.id)
            }
            for (c = 0; c < V.length; c++) {
                ...
                var W = [];
                for (w = _.children.length - 1; 0 <= w; w--) {
                    if (0 == (j = _.children[w]).childrenIds.length && !j.isAssistant && "split" != j.templateName && j.parent && "split" != j.parent.templateName) {
                        if ((j = _.children[w]).parent = null,
                            j.layout = OrgChart.tree,
                            _.children.splice(w, 1),
                            _.childrenIds.splice(w, 1),
                            t.layout == OrgChart.treeRightOffset && W.splice(0, 0, j.id),
                            t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
                            var J;
                            I = "transperant_" + j.templateName;
                            OrgChart.templates[I] = {
                                linkAdjuster: {},
                                link: "",
                                node: "",
                                ...
                                (J = new OrgChart.node(j.id + "_transperant", null, null, [], I)).layout = OrgChart.tree,
                                n[J.id] = J,
                                W.splice(0, 0, J.id)
                        }
                        t.layout != OrgChart.treeRightOffset && W.splice(0, 0, j.id)
                ...
                for (E = 1,
                    w = W.length - 1; 0 <= w; w--) {
                    if (w % 2 && w != W.length - 1)
                        (M = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split")).layout = OrgChart.tree,
                            n[M.id] = M,
                            W.splice(w, 0, M.id),
                            E++;
                    else
                        w % 2 && W.splice(w, 0, k.id)
                }
        ...
        if (r == OrgChart.action.init && t) {
            if (t.tags)
                for (var l = 0; l < t.tags.length; l++) {
                    var s = t.tags[l];
                    i.tags && i.tags[s] && null != i.tags[s].state && (i.tags[s].state == OrgChart.COLLAPSE && o.push(t.id),
                        t.state = i.tags[s].state)
                }
            if (i.collapse.level) {
                for (var h = 0, d = e[t.pid]; null != d && h <= i.collapse.level;)
                    d = e[d.pid],
                        h++;
                if (h >= i.collapse.level) {
                    ...
                    if (Array.isArray(i.expand.nodes))
                        if (i.expand.allChildren) {
                            ...
                            c = -1 != i.expand.nodes.indexOf(t.id);
                    !c && i.collapse.allChildren ? (o.push(t.id),
                        t.state = OrgChart.COLLAPSE) : c || i.collapse.allChildren || h != i.collapse.level || (o.push(t.id),
                ...
                switch (r) {
                    ...
                    case OrgChart.orientation.bottom_left:
                        ...
                        a = h * l + i.groupNodesSeparation * l + i.groupNodesSeparation + i.groupPadding[1] + i.groupPadding[3],
                            n = d * s + i.groupNodesSeparation * s + i.groupNodesSeparation + i.groupPadding[0] + i.groupPadding[2];
                        break;
                    case OrgChart.orientation.right:
                    case OrgChart.orientation.left:
                    case OrgChart.orientation.right_top:
    ...
    OrgChart.server._setGroupNodes = function (t, e) {
        if (t.isGroup && t.groupState == OrgChart.EXPAND) {
            ...
            if (0 == r.layout_type)
                switch (e) {
                    ...
                    case OrgChart.orientation.bottom_left:
                        for (var n = 0, o = 0; o < t.bgnodes.length; o++) {
                            (s = t.bgnodes[o]).x = t._groupSettings._cellWidth / 2 - s.w / 2,
                    ...
                    case OrgChart.orientation.left_top:
                        ...
                        for (o = 0; o < t.bgnodes.length; o++) {
                            (s = t.bgnodes[o]).y = t._groupSettings._cellHeight / 2 - s.h / 2,
                ...
                for (o = 0; o < t.bgnodes.length; o++) {
                    var s;
                    (s = t.bgnodes[o]).x = t._groupSettings._cellWidth / 2 - s.w / 2 + r.groupNodesSeparation + (t._groupSettings._cellWidth * i + r.groupNodesSeparation * i),
                        s.y = t._groupSettings._cellHeight / 2 - s.h / 2 + r.groupNodesSeparation + (t._groupSettings._cellHeight * a + r.groupNodesSeparation * a),
    ...
    OrgChart.server._getResponse = function (t, e, r, i, a, n, o, l, s, h, d, c, g, p, f) {
        var u = {}
            ...
            , O = a - o + 2 * i.padding
            , y = n - l + 2 * i.padding
            , v = OrgChart.getScale(s, t, e, i.scaleInitial, O, y);
        if (C.top = l - i.padding,
            C.left = o - i.padding,
            C.bottom = n + i.padding - e / v,
            C.right = a + i.padding - t / v,
            C.maxX = a,
            C.minX = o,
            C.maxY = n,
            C.minY = l,
            ...
            if (x - 2 * i.padding >= a - o)
                ...
                switch (w = h.x - x / 2 + OrgChart.server._getNodeWidth(h, i) / 2,
                i.orientation) {
                    case OrgChart.orientation.right:
                    case OrgChart.orientation.right_top:
            ...
            if (b - 2 * i.padding >= n - l)
                ...
                switch ((_ = -(b / 2 - (n - l) / 2)) > -i.padding && (_ = -i.padding),
                i.orientation) {
                    case OrgChart.orientation.bottom:
                    case OrgChart.orientation.bottom_left:
        ...
        for (var N in g) {
            ...
            if (OrgChart.server._isVisible(A, i, s, d)) {
                ...
                null != I && (I.x == A.x && I.y == A.y || (m[0].push(A.id),
                    m[1].push({
    ...
    OrgChart.ui = {
        ...
        defs: function () {
            ...
            for (var e in OrgChart.templates) {
                ...
                r.defs ? (OrgChart.ui._defsIds[e] = OrgChart._randomId(),
                    t += r.defs.replace("{randId}", OrgChart.ui._defsIds[e])) : t += r.defs
            }
            return "<defs>" + t + "</defs>"
        },
        css: function () {
            ...
            if (d.defs && (c = c.replace("{randId}", OrgChart.ui._defsIds[t.templateName])),
                ...
                if (g)
                    g = OrgChart._wrapText(g, d.groupName),
                        c += u = (u = d.groupName.replaceAll("{val}", g)).replaceAll("{randId}", OrgChart._randomId()).replaceAll("{randId2}", OrgChart._randomId());
                c += u
            } else
                for (var p in o) {
                    var f = o[p];
                    if (e) {
                        var u, C = e[f];
                        if ("function" == typeof f && (C = f(h, t)),
                            null != C && null != C && null != d[p])
                            c += u = (u = (OrgChart._lblIsImg(i, p) || "string" != typeof C || (C = OrgChart._wrapText(C, d[p])),
                                d[p].replaceAll("{val}", C))).replaceAll("{randId}", OrgChart._randomId()).replaceAll("{randId2}", OrgChart._randomId())
                    }
                }
        ...
        expandCollapse: function (t, e, r, i) {
            if (r === OrgChart.action.exporting)
                return "";
            if (0 == t.childrenIds.length)
                return "";
            if (t.isAssistant)
                return "";
            if ("split" == t.templateName)
                return "";
            var a = ""
                , n = 0
                , o = 0
            ...
            return n -= l.expandCollapseSize / 2,
                o -= l.expandCollapseSize / 2,
                t.collapsedChildrenIds.length ? (a += OrgChart.expcollOpenTag.replace("{id}", t.id).replace("{x}", n).replace("{y}", o),
                    a += l.plus) : (a += OrgChart.expcollOpenTag.replace("{id}", t.id).replace("{x}", n).replace("{y}", o),
                        a += l.minus),
                ...
                if (t[s]) {
                    ...
                    for (var f = e; null == h;) {
                        var u = !1
                            ...
                            , m = C.leftNeighbor
                            , O = C.rightNeighbor;
                        if (C.id == s ? h = C : OrgChart._intersects(C, p, r) && (p = OrgChart._addPoint(C, l, r, p, d),
                            u = !0),
                            C.id != s) {
                            for (; m;) {
                                if (m.id == s) {
                                ...
                                OrgChart._intersects(m, p, r) && (p = OrgChart._addPoint(m, l, r, p, d),
                                    u = !0),
                                    m = m.leftNeighbor
                            }
                            for (; O;) {
                                ...
                                OrgChart._intersects(O, p, r) && (p = OrgChart._addPoint(O, l, r, p, d),
                                    u = !0),
                                    O = O.rightNeighbor
                            }
                        }
                        if (!u) {
                            var y = l[l.length - 1][0]
                                , v = 0;
                            if (C.parent)
                                switch (g = r.levelSeparation,
                                C.parent.layout != OrgChart.mixed && C.parent.layout != OrgChart.tree || (g = r.mixedHierarchyNodesSeparation),
                                r.orientation) {
                                    case OrgChart.orientation.top:
                                    case OrgChart.orientation.top_left:
                                        v = C.parent.y + C.parent.h + g * (2 / 3);
                                        break;
                                    case OrgChart.orientation.bottom:
                                    case OrgChart.orientation.bottom_left:
                                        v = C.parent.y - g * (2 / 3);
                    ...
                    switch (c = OrgChart.t(h.templateName, a),
                    l.splice(l.length - 1, 1),
                    r.orientation) {
                        case OrgChart.orientation.top:
                        case OrgChart.orientation.top_left:
                            l.push([h.x + h.w / 2 + c.expandCollapseSize, l[l.length - 1][1]]),
                        ...
                        case OrgChart.orientation.left:
                        case OrgChart.orientation.left_top:
                            l.push([l[l.length - 1][0], h.y + h.h / 2 + c.expandCollapseSize]),
                    ...
                    for (var x = 1; x < l.length; x++)
                        l[x] = "L" + l[x].join(",");
                    l = l.join(" "),
                        n += OrgChart.secondLinkOpenTag.replace("{id}", e.id).replace("{child-id}", s) + c.secondLink.replace("{d}", l),
        ...
        link: function (t, e, r, i) {
            ...
            for (var l = 0; l < t.children.length; l++) {
                ...
                d = d.replace("{xa}", h.xa).replace("{ya}", h.ya).replace("{xb}", h.xb).replace("{yb}", h.yb).replace("{xc}", h.xc).replace("{yc}", h.yc).replace("{xd}", h.xd).replace("{yd}", h.yd),
                    n.push(OrgChart.linkOpenTag.replace("{id}", t.id).replace("{class}", "link " + s.tags.join(" ")).replace("{level}", t.level).replace("{child-id}", s.id)),
                    n.push(d);
                var c = "";
    ...
    OrgChart.getScale = function (t, e, r, i, a, n) {
        if (t || i !== OrgChart.match.boundary)
            return t || i !== OrgChart.match.width ? t || i !== OrgChart.match.height ? t ? (o = e / t[2],
                (l = r / t[3]) < o ? l : o) : i : r / n : e / a;
        var o = e / a
            , l = r / n;
        return l < o ? l : o
    }
    ...
    OrgChart._wrapText = function (t, e) {
        ...
        if ("multiline" == s) {
            ...
            for (var m = 1; m < p.length; m++) {
                ...
                if (u.firstChild.data += " " + p[m],
                    u.getComputedTextLength() > l)
                    u.firstChild.data = u.firstChild.data.slice(0, O),
                        (u = document.createElementNS(h, "tspan")).setAttributeNS(null, "x", a),
            ...
            if (null != d.innerHTML)
                ...
                for (m = d.childNodes.length - 1; 0 <= m; m--)
                    v = XMLSerializer().serializeToString(d.childNodes[m]) + v,
    ...
    OrgChart._json2csv = function (t) {
        for (var n = [], e = function (t) {
            ...
            for (var a in t[i])
                OrgChart._arrayContains(n, a) || (n.push(a),
                    r += a + ",");
        r += "\n";
    ...
    OrgChart.loc.ppdfSize = "Size",
    OrgChart.loc.ppdfFitToDrwaing = "Fit To Drwaing",
    ...
    OrgChart.pdfPrevUI.show = function (r, i) {
        ...
        t.id = "bg-ppdf-btns",
            ...
            t.innerHTML = "<h1>" + OrgChart.loc.ppdfCmdTitle + '</h1><div><button id="bg-prev-save" style="font-size: 14px; width: 90px;">' + OrgChart.loc.ppdfSave + '</button>&nbsp;<button id="bg-prev-cancel" style="width: 90px;font-size: 14px;">' + OrgChart.loc.ppdfCancel + '</button></div><div style="margin-top:30px; height:10px;border-bottom:1px solid #eeeeee;"></div><div style="padding-top:30px;"><label for="bg-size">' + OrgChart.loc.ppdfSize + ': </label><select id="bg-ppdf-size" style="color: #757575; width: 150px; font-size: 14px;" id="bg-size"><option value="fit">' + OrgChart.loc.ppdfFitToDrwaing + '</option><option value="A4">A4</option></select></div><div style="padding-top:10px;"><label for="bg-ppdf-layout">' + OrgChart.loc.ppdfLayout + ': </label><select id="bg-ppdf-layout" style="color: #757575; width: 150px;font-size: 14px;" ><option value="false">' + OrgChart.loc.ppdfPortrait + '</option><option value="true">' + OrgChart.loc.ppdfLandscape + '</option></select></div><div style="padding-top:10px;"><label for="bg-scale">Scale: </label><select id="bg-ppdf-scale" style="color: #757575; width: 150px;font-size: 14px;" id="bg-scale"><option value="fit">' + OrgChart.loc.ppdfFittopagewidth + '</option><option value="100">100%</option></select></div><div style="margin-top:10px;margin-bottom:10px; height:10px;border-bottom:1px solid #eeeeee;"></div><div style="padding-top:10px;"><label for="bg-ppdf-header">' + OrgChart.loc.ppdfHeader + ': </label><input id="bg-ppdf-header" type="text" style="color: #757575; width: 100px;font-size: 14px;" ></div><div style="padding-top:10px;"><label for="bg-ppdf-footer">' + OrgChart.loc.ppdfFooter + ': </label><input id="bg-ppdf-footer" type="text" style="color: #757575; width: 100px;font-size: 14px;" ></div><div style="padding-top:10px;"><label for="bg-ppdf-margin">' + OrgChart.loc.ppdfMargin + ': </label><input id="bg-ppdf-margin" type="text" style="color: #757575; width: 100px;font-size: 14px;" ></div>';
        var e = document.createElement("div");
        ...
        var a, n, o, l = r.element.querySelector("#bg-ppdf-size"), s = r.element.querySelector("#bg-ppdf-layout"), h = r.element.querySelector("#bg-ppdf-scale"), d = r.element.querySelector("#bg-ppdf-margin"), c = r.element.querySelector("#bg-ppdf-header"), g = r.element.querySelector("#bg-ppdf-footer");
        l.value = i.format,
            s.value = i.landscape,
            h.value = i.scale,
            d.value = i.margin,
    ...
    OrgChart.pdfPrevUI._showHide = function (t, e, r) {
        "fit" == t.value ? (e.parentNode.style.display = "none",
            r.parentNode.style.display = "none") : (e.parentNode.style.display = "block",
    ...
    OrgChart.pdfPrevUI._handler = function (t, e, r) {
        ...
        for (var o = n.querySelector("svg"), l = t.element.querySelector("#bg-ppdf-content"), s = 0; s < a.pages.length; s++) {
            ...
            Object.assign(h.style, {
                ...
                l.appendChild(h),
                o && o.style.backgroundColor ? h.style.backgroundColor = o.style.backgroundColor : h.style.backgroundColor = "#fff";
            var d = h.contentWindow.document;
            d.open(),
                "A4" == i.format && i.landscape ? (h.style.width = OrgChart.A4h + "px",
                    h.style.height = OrgChart.A4w + "px") : "A4" == i.format ? (h.style.width = OrgChart.A4w + "px",
                        h.style.height = OrgChart.A4h + "px") : "fit" == i.format && (h.style.width = a.size.w + "px",
                            h.style.height = a.size.h + "px",
                            l.style.marginTop = 0,
                            l.style.marginBottom = 0,
                            h.style.margin = "0 auto"),
                a.pages[s].html ? (a.pages[s].backgroundColor && (h.style.backgroundColor = a.pages[s].backgroundColor),
                    d.write(OrgChart._exportHtml(a.pages[s].html, i, a, s + 1, a.pages.length))) : (o.setAttribute("viewBox", a.pages[s].vb),
                        d.write(OrgChart._exportHtml(n.innerHTML, i, a, s + 1, a.pages.length)));
            var c = d.getElementById("bg-header")
                , g = d.getElementById("bg-footer");
            c.style.top = i.margin[0] - c.offsetHeight - 7 + "px",
                g.style.bottom = i.margin[2] - g.offsetHeight - 7 + "px",
                d.close()
        }
        var p = t.element.querySelector("#bg-ppdf-content");
        OrgChart.anim(p, {
            opacity: 0
        }, {
                opacity: 1
    ...
    ,
    OrgChart._exportHtml = function (t, e, r, i, a) {
        for (var n = "", o = 0; o < e.margin.length; o++)
            n += e.margin[o] + "px ";
        return '<!DOCTYPE html><html style="margin:0;padding:0;overflow:auto;"><head></head><body style="margin:' + n + ";padding:0;overflow:hidden;width:" + r.innerSize.w + "px;height:" + r.innerSize.h + 'px;"><div id="bg-header" style="color:#757575;position:absolute;left:' + e.margin[3] + 'px;">' + e.header.replace("{current-page}", i).replace("{total-pages}", a) + "</div>" + t + '<div id="bg-footer" style="color:#757575;position:absolute;left:' + e.margin[3] + 'px;">' + e.footer.replace("{current-page}", i).replace("{total-pages}", a) + "</div></body></html>"
    }
    ,
